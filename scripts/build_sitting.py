...
from karbes import atlas as A, chairpage, page, sitting
...
raw = json.loads(src.read_text(encoding="utf-8")); start = raw[0]["agendaItems"][0]["events"][0]["date"]
s = sitting.load(src); miss = sitting.attach_tone(s)
d = json.loads(recp.read_text(encoding="utf-8"))
rec = Recording(date=d["date"], reactions=[Reaction(**r) for r in d["reactions"]], group_hz=d["group_hz"],
                raster=d["raster"], frame_event=d["frame_event"], onset=d.get("onset", []))
b = chairpage.bundle(s, rec, A.load(Path("data/malecns")), start)
print(s.date, "| unscored", miss, "|", {k: b["summary"][k] for k in ("stimuli", "fly_bells", "chair_order", "chair_bell", "chair_time", "coincide", "heckles", "hostile", "bio_seconds")})
